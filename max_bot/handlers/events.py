...
async def handle_create_event_callback(update: MaxUpdate, context: MaxContext) -> None:
    ...
    if data.startswith("create_event_begin_"):
        ...
        await query.edit_message_text(
            text=tr(
                "Добавь пользователя: отправь @username, ссылку t.me/... или ID. Можно и через 📎 Контакт.",
                locale,
            ),
...
async def show_upcoming_events(update: MaxUpdate, context: MaxContext) -> None:
    ...
    if message:
        ...
        attachments = reply_markup.to_attachments() if reply_markup else None
        await context.bot.send_message(text=text, user_id=user.id, attachments=attachments, fmt="html")
